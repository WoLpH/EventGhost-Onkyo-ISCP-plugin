...
eg.RegisterPlugin(
    name='Onkyo ISCP',
    author='Wolph (Rick van Hattem), Alexander Hartmaier, Sem;colon',
    version='1.0',
    kind='external',
    guid='{5B3B8AEB-08D7-4FD0-8BEE-8FE50C231E09}',
    description='Controls any Onkyo Receiver which supports the EISCP '
    '(networked ISCP) protocol.',
...
class OnkyoISCP(eg.PluginBase):
    ...
    def Receive(self):
        while not self.stopThreadEvent.is_set():
            try:
                ...
                # the first element of the returned list is a list of readable
                # sockets
                if ready[0]:
                    ...
                    if version != self.version:
                        self.PrintError(
                            'OnkyoISCP: ISCP version %s not supported' %
                            version)
                        return
                    ...
                    # message ends in EOL CR LF, is three chars shorter than
                    # data size
                    message = reply[
                        headersize:headersize + datasize].rstrip('\x1a\r\n')
    ...
    def Connect(self):
        ...
        try:
            ...
            self.PrintError(
                'OnkyoISCP: Failed to connect to %s:%s: %r' % (ip, port, e))
        else:
...
class SendCommand(eg.ActionBase):
    ...
    def __call__(self, Command):
        ...
        try:
            ...
            self.PrintError(
                'OnkyoISCP: Error sending command, retrying: %r' % msg)
            ...
            try:
                ...
                self.PrintError('OnkyoISCP: Error sending command: %r' % msg)
